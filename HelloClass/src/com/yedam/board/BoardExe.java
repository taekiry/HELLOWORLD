...
public class BoardExe { // 기본 메소드들을 저장해뒀다가 App class에서 사용
	...
	/*
	 * 1page : [0]~[4] => (page-1) * 5 <= x < page * 5 2page : [5]~[9] ....
	 */
...
//} 내가짠 로그인체크 코드(메소드에 넣기전)
		...
		for (int i = 1; i <= 3; i++) {
			...
			if (!UserExe.login(id, pw)) {
				...
				if (i == 3) {
					...
					return false; // 아무 반환 값 없으니 login 메소드 자체를 종료;
				}
				continue; // 실패의경우 다시 for문으로 i = 3이 될때까지
			}
			return true; // 로그인을 성공하면 언제라도 반복문을 빠져 나와서 아래 코드를 실행.
		} // 3번의 기회를 제공.
		return false;// loginCheck가 오류떠서 넣은거 왜넣은지는 모르겠음.
	}// end boolean loginCheck
	...
	void execute() {
		...
		while (run) {
			...
			System.out.print("선택>> ");
			int selectNo = 0;
			//문자를 숫자로 변경하면서 예외발생가능.
			try {
				selectNo = Integer.parseInt(scn.nextLine()); // 예외처리 : 만약 a 입력하면 parseInt처리 못하니까
			} catch(NumberFormatException e) {
				System.out.println("1,2,3,4,5 중에 선택하세요.");
				continue; //return; 아님. while문에서 return 쓰는 경우 찾아보자!
			}
...
//		int no = userMenu("글번호를 입력하세요");
	...
	void boardList() {
		...
		while (true) {
			...
			System.out.println("------------------------------------------------");
			System.out.println("상세보기: 다음페이지(n),이전페이지(p),글번호입력, 메뉴로 이동(q)");
			System.out.println("------------------------------------------------");
			...
			if (str.equals("q")) {
				...
				} catch (NumberFormatException e) { //NFE 실행오류 숫자안쓰면
